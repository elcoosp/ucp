...
mod tests {
    ...
    fn empty_spec() -> SynthesisOutput {
        SynthesisOutput {
            ...
            components: vec![],
            stats: PipelineStats {
                files_scanned: 5, files_parsed: 3, components_found: 0,
                conflicts_detected: 0, llm_enriched: false,
            },
        }
    }

    fn make_component(id: &str, prop_name: &str, prop_type: AbstractPropType) -> CanonicalAbstractComponent {
    ...
    fn make_component_with_source(id: &str, prop_name: &str, prop_type: AbstractPropType, file_path: &str) -> CanonicalAbstractComponent {
    ...
    #[test]
    fn merge_empty_specs() { assert!(merge_specs(&[]).unwrap().components.is_empty()); }
    #[test]
    fn merge_single_spec_passthrough() { assert_eq!(merge_specs(&[empty_spec()]).unwrap().stats.files_scanned, 5); }
    #[test]
    fn merge_two_specs_accumulates_stats() {
        let spec2 = SynthesisOutput {
            ucp_version: "4.0.0".to_string(), components: vec![],
            stats: PipelineStats { files_scanned: 10, files_parsed: 7, components_found: 0, conflicts_detected: 0, llm_enriched: true },
        };
    ...
    fn dedup_same_components_single_output() {
        ...
        let spec = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![comp], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
    ...
    fn dedup_same_components_different_sources() {
        let a = make_component_with_source("Button", "label", AbstractPropType::StaticValue(Box::new(AbstractPropType::Any)), "leptos/src/button.rs");
        let b = make_component_with_source("Button", "label", AbstractPropType::StaticValue(Box::new(AbstractPropType::Any)), "react/src/Button.tsx");
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![a], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![b], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
    ...
    fn dedup_keeps_higher_confidence() {
        let mut a = make_component("Button", "label", AbstractPropType::StaticValue(Box::new(AbstractPropType::Any)));
        a.props[0].confidence = 0.7;
        let mut b = make_component("Button", "label", AbstractPropType::StaticValue(Box::new(AbstractPropType::Any)));
        b.props[0].confidence = 0.95;
        b.source_repos = vec![SourceAttribution { repo_url: "local".to_string(), file_path: "other.rs".to_string(), line_start: 1 }];
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![a], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![b], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        assert_eq!(merge_specs(&[s1, s2]).unwrap().components[0].props[0].confidence, 0.95);
    }
    ...
    fn dedup_keeps_state_machine_from_other() {
        ...
        b.extracted_state_machine = Some(StateMachine { id: "sm".to_string(), initial: "Closed".to_string(), states: std::collections::BTreeMap::new() });
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![a], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![b], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        assert!(merge_specs(&[s1, s2]).unwrap().components[0].extracted_state_machine.is_some());
    }
    ...
    fn dedup_keeps_extracted_parts_from_other() {
        ...
        b.extracted_parts = vec![ExtractedPart { name: "children".to_string(), selectable: true }];
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![a], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![b], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        assert!(!merge_specs(&[s1, s2]).unwrap().components[0].extracted_parts.is_empty());
    }

    #[test]
    fn dedup_merges_events() {
        let mut a = make_component("Form", "data", AbstractPropType::Any);
        a.events = vec![CanonicalAbstractEvent { canonical_name: "Submit".to_string(), abstract_payload: AbstractPropType::AsyncEventHandler(vec![]) }];
        let mut b = make_component("Form", "data", AbstractPropType::Any);
        b.events = vec![CanonicalAbstractEvent { canonical_name: "Reset".to_string(), abstract_payload: AbstractPropType::AsyncEventHandler(vec![]) }];
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![a], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![b], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        assert_eq!(merge_specs(&[s1, s2]).unwrap().components[0].events.len(), 2);
    }

    #[test]
    fn dedup_deduplicates_events() {
        let mut a = make_component("Form", "on_submit", AbstractPropType::AsyncEventHandler(vec![]));
        a.events = vec![CanonicalAbstractEvent { canonical_name: "Submit".to_string(), abstract_payload: AbstractPropType::AsyncEventHandler(vec![]) }];
        let mut b = make_component("Form", "on_submit", AbstractPropType::AsyncEventHandler(vec![]));
        b.events = vec![CanonicalAbstractEvent { canonical_name: "Submit".to_string(), abstract_payload: AbstractPropType::AsyncEventHandler(vec![]) }];
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![a], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![b], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        assert_eq!(merge_specs(&[s1, s2]).unwrap().components[0].events.len(), 1);
    }

    #[test]
    fn merge_detects_cross_spec_type_conflict_after_dedup() {
        let c1 = make_component("Button", "disabled", AbstractPropType::ControlFlag);
        let c2 = make_component("Button", "disabled", AbstractPropType::StaticValue(Box::new(AbstractPropType::Any)));
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![c1], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![c2], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        assert!(merge_specs(&[s1, s2]).unwrap().stats.conflicts_detected > 0);
    }

    #[test]
    fn merge_unrelated_components_no_conflicts() {
        let a = make_component("Button", "label", AbstractPropType::StaticValue(Box::new(AbstractPropType::Any)));
    ...
    fn merge_no_conflict_for_matching_types_after_dedup() {
        let c1 = make_component("Input", "value", AbstractPropType::ControlledValue(Box::new(AbstractPropType::Any)));
        let c2 = make_component("Input", "value", AbstractPropType::ControlledValue(Box::new(AbstractPropType::Any)));
        let s1 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![c1], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };
        let s2 = SynthesisOutput { ucp_version: "4.0.0".to_string(), components: vec![c2], stats: PipelineStats { files_scanned: 1, files_parsed: 1, components_found: 1, conflicts_detected: 0, llm_enriched: false } };

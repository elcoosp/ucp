...
// StructComponentVisitor and GpuiComponentVisitor need the new fields too.
// We'll include them with default values.
...
fn extract_type_name(ty: &syn::Type) -> String {
    match ty {
        syn::Type::Path(type_path) => type_path.path.segments.last().map(|seg| seg.ident.to_string()).unwrap_or_default(),
        _ => String::new(),
    }
}
...
impl Visit<'_> for StructComponentVisitor {
    fn visit_item_struct(&mut self, item: &syn::ItemStruct) {
        ...
        for field in &item.fields {
            ...
            props.push(RawPropExtraction { name: field_name, raw_type, has_default, is_event: false, is_spread_attributes: false });
        }
        ...
        if let Some((struct_name, props, line_start)) = self.props_structs.remove(&impl_type_name) {
            ...
            for item_impl in &item.items {
                if let syn::ImplItem::Fn(method) = item_impl {
                    if method.sig.ident == "render" && matches!(method.vis, syn::Visibility::Public(_)) && method.sig.inputs.len() == 1 {
...
impl GpuiComponentVisitor {
    pub fn extract(code: &str) -> Result<Vec<RawComponentExtraction>> {
        let ast = syn::parse_file(code).map_err(|e| ucp_core::UcpError::Parsing(e.to_string()))?;
        ...
        for (_, item_struct) in struct_candidates {
            ...
            for field in &item_struct.fields {
                ...
                let raw_type = crate::utils::normalize_type_string(&field.ty.to_token_stream().to_string());
                let has_default = raw_type.trim().starts_with("Option");
                props.push(RawPropExtraction { name: field_name.clone(), raw_type, has_default, is_event: false, is_spread_attributes: false });
            }
                ...
                for item_impl in methods {
                    if let syn::ImplItem::Fn(method) = item_impl {
                        ...
                        if !matched_field && is_event {
                            props.push(RawPropExtraction { name: method.sig.ident.to_string(), raw_type: arg_type, has_default: false, is_event: true, is_spread_attributes: false });
                        }
                    }
                }
            }
            ...
            {
                props.push(RawPropExtraction { name: "children".to_string(), raw_type: "Children".to_string(), has_default: false, is_event: false, is_spread_attributes: false });
            }
    ...
    fn group_variant_props(props: &mut Vec<RawPropExtraction>) {
        ...
        for (field, variants) in field_setters.iter() {
            if variants.len() > 1 {
                ...
                props.push(RawPropExtraction { name: field.clone(), raw_type: format!("enum {{ {} }}", enum_values), has_default: default.is_some(), is_event: false, is_spread_attributes: false });
            }
        }
    }
}

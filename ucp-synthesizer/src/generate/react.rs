...
/// Generate React TypeScript component code from a package manifest.
pub fn generate_react(manifest: &PackageManifest, output_dir: &str) -> Result<()> {
    ...
    for comp in &manifest.components {
        ...
        let file_path = dir.join("src").join(format!("{}.tsx", file_name));
...
fn generate_component_code(comp: &CanonicalAbstractComponent) -> String {
    ...
    for prop in &comp.props {
        let ts_type = map_cam_to_ts(prop);
        ...
        let default_val = if prop.reactivity == AbstractReactivity::Static {
            if prop.concrete_type.as_deref() == Some("bool") {
                " = false"
            } else {
                ""
            }
...
fn map_cam_to_ts(prop: &CanonicalAbstractProp) -> String {

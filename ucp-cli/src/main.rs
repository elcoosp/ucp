...
enum Commands {
    ...
    /// Generate an interactive visual dashboard from a spec file
    Dashboard {
        #[arg(long)]
        spec: PathBuf,
    ...
    /// Export AI-optimized JSON contract from a spec file
    Contract {
        #[arg(long)]
        spec: PathBuf,
...
fn cmd_contract(spec: &Path, output: &Path) -> anyhow::Result<()> {
    ...
    ucp_synthesizer::contract::ai_contract::export_ai_contract(&synth, &output.to_string_lossy())
        .context("Failed to export AI contract")?;
...
fn cmd_generate(spec: &Path, target: &str, output: &Path) -> anyhow::Result<()> {
    ...
    match target {
        ...
        "shadcn-registry" => {
            ...
            .context("Failed to generate registry files")?;
